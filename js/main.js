...
const cardNumberDispley = document.querySelectorAll(".card-number-display");
...
cardNumber.addEventListener("input", () => {
	...
	for (let i = 0; i < cardNumberDispley.length; i++) {
		if (i < inputNumber.length) {
			cardNumberDispley[i].textContent = inputNumber[i];
		} else {
			cardNumberDispley[i].textContent = "_";
		}
	}

	if (inputNumber.length <= cardNumberDispley.length) {
		currentSpanIndex = inputNumber.length;
	} else {
		currentSpanIndex = cardNumberDispley.length;
	}
});
